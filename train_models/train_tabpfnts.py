...
def main(args):
    ...
    results_dir = f'{args.results_dir}/{args.dataset}_{args.horizon}/tabpfnts/trial_{args.experiment_id}'
    os.makedirs(results_dir, exist_ok = True)

    # Initialize TabPFNTS model
    # Note: TabPFNTS does not require hyperparameter tuning like other models.
    tabpfn_model = TabPFNTS(h=args.horizon,
                            input_size=args.input_size,
                            context_length=4096,
                            tabpfn_mode=TabPFNMode.LOCAL if TabPFNMode else None,
                            debug=True,
                            loss=MAE())
...
def parse_args():
    desc = "Example of TabPFNTS evaluation"
